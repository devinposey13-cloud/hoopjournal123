...
// Add watermark logo to a PDF page (decorative branding watermark)
...
// Add "Generated by Hoop Journal" watermark for free users
function addFreeUserWatermark(doc: jsPDF) {
  const pageWidth = doc.internal.pageSize.getWidth();
  const pageHeight = doc.internal.pageSize.getHeight();
  ...
  const text = 'Generated by Hoop Journal';
  const textWidth = doc.getTextWidth(text);
  
  // Draw at center with rotation
  doc.text(text, centerX - textWidth / 2, centerY, { angle: 25 });
  
  doc.restoreGraphicsState();
  doc.setTextColor(0, 0, 0);
}

// Add viral footer to every page
function addViralFooter(doc: jsPDF) {
  const pageWidth = doc.internal.pageSize.getWidth();
  const pageHeight = doc.internal.pageSize.getHeight();
  ...
  doc.text('Track your game at hoopjournal.me', pageWidth / 2, pageHeight - 6, { align: 'center' });
  doc.setTextColor(0, 0, 0);
}
